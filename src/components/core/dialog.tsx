...
function DialogTrigger({ children, className, asChild }: DialogTriggerProps) {
  ...
  if (asChild) {
    return (
      <div className={className} onClick={context.handleTrigger}>
        {children}
      </div>
    );
  }
    ...
    <AnimatePresence mode="wait">
      {isOpen && (
        <motion.dialog
          ...
          className={cn(
            'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform rounded-lg p-0',
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'hover:opacity-100',
        'focus:ring-zinc-500 focus:ring-offset-2 disabled:pointer-events-none text-foreground',
        className
      )}
      disabled={disabled}
    >
